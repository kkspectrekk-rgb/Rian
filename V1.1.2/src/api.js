...
export async function readLocalMetadata(filePath) {
  return bridge?.readLocalMetadata ? bridge.readLocalMetadata(filePath) : { ok: false, message: '请使用 Rain 桌面版读取本地歌曲信息' };
}
...
export async function getQQPlaylist(id) {
  return bridge?.qqPlaylist ? bridge.qqPlaylist(id) : { ok: false, message: '请使用 Rain 桌面版读取 QQ 歌单' };
}

export async function getCacheInfo() {
  return bridge?.getCacheInfo ? bridge.getCacheInfo() : { ok: false, path: '' };
}

export async function setCachePath(nextPath) {
  return bridge?.setCachePath ? bridge.setCachePath(nextPath) : { ok: false, message: '请使用 Rain 桌面版更改缓存路径' };
}

export async function clearCache() {
  return bridge?.clearCache ? bridge.clearCache() : { ok: false, message: '请使用 Rain 桌面版清理缓存' };
}
...
function coverUrl(item) {
  let cover = first(
    ...
    item?.al?.picUrl, item?.album_img,
  );
  if (typeof cover === 'string' && cover.includes('{size}')) cover = cover.replaceAll('{size}', '400');
  if (cover) return cover;
  const albumMid = first(item?.albummid, item?.albumMid, item?.album?.mid, item?.album?.albumMid, item?.album?.id);
