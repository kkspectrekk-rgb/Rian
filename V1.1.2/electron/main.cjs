...
const localCoverDirectory = () => path.join(app.getPath('userData'), 'rain-local-covers');
const defaultCacheDirectory = () => app.isPackaged
  ? path.join(path.dirname(process.execPath), 'RainCache')
  : path.join(app.getPath('userData'), 'rain-media-cache');
...
function embeddedCoverUrl(picture, filePath) {
  ...
  const directory = localCoverDirectory();
  const filename = `${digest}${extension}`;
...
async function readLocalMetadata(filePath) {
  ...
  return {
    ...
    sampleRate: Number(metadata.format?.sampleRate || 0),
    bitsPerSample: Number(metadata.format?.bitsPerSample || 0),
    format: String(metadata.format?.codec || metadata.format?.container || ''),
...
async function fetchQQPlaylist(id) {
  ...
  url.searchParams.set('disstid', String(id));
  ...
  const response = await fetch(url, { headers: { Referer: 'https://y.qq.com/' }, signal: AbortSignal.timeout(15000) });
  if (!response.ok) throw new Error(`QQ 歌单接口响应 ${response.status}`);
  const data = await response.json();
  const disc = data?.cdlist?.[0];
  if (!disc) throw new Error('没有读取到 QQ 歌单');
  const tracks = (disc.songlist || []).map((item, index) => ({
    source: 'qq',
    id: String(item.songid || item.songmid || index + 1),
    mid: item.songmid || item.strMediaMid || '',
    n: Number(index + 1),
    title: item.songname || item.songorig || '未知歌曲',
    artist: (item.singer || []).map((singer) => singer.name || '').filter(Boolean).join(' / ') || '未知艺人',
  ...
  return {
    id: String(disc.disstid || id),
    source: 'qq',
    title: disc.dissname || 'QQ 歌单',
    cover: disc.logo || '',
    tracks,
  };
}

function readDirectorySize(directory) {
  ...
  while (stack.length) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) stack.push(entryPath);
        ...
        try { total += fs.statSync(entryPath).size; } catch {}
      }
    }
  }
  return total;
...
async function startApplication() {
  ...
  protocol.handle('rain-cache', (request) => {
    try {
      ...
      let filePath = path.join(mediaCachePath(), kind, filename);
      if (!fs.existsSync(filePath) && kind === 'cover') {
        filePath = path.join(localCoverDirectory(), filename);
      }
  ...
  ipcMain.handle('qq:playlist', async (_event, id) => {
    if (typeof id !== 'string' || !/^\d+$/.test(id)) return { ok: false, message: 'QQ 歌单 ID 无效' };
    try {
      return { ok: true, playlist: await fetchQQPlaylist(id) };
    } catch (error) {
      return { ok: false, message: error.message || 'QQ 歌单读取失败' };
    }
  });

  ipcMain.handle('cache:get-info', () => {
    const directory = mediaCachePath();
    try { fs.mkdirSync(directory, { recursive: true }); } catch {}
    return { ok: true, path: directory, size: readDirectorySize(directory) };
  });

  ipcMain.handle('cache:set-path', (_event, nextPath) => {
    if (typeof nextPath !== 'string' || !path.isAbsolute(nextPath)) return { ok: false, message: '缓存路径必须是绝对路径' };
    try {
      fs.mkdirSync(nextPath, { recursive: true });
      writeSettingsData({ ...readSettingsData(), cacheDirectory: nextPath });
      return { ok: true, path: nextPath };
    } catch (error) {
      return { ok: false, message: error.message || '无法更改缓存路径' };
    }
  });
  ...
  ipcMain.handle('cache:clear', () => {
    ...
    try {
      ...
      writeCacheIndex({ tracks: {}, searches: {} });
      return { ok: true, path: directory };
    } catch (error) {
      return { ok: false, message: error.message || '清理缓存失败' };
    }
  });
  ...
  ipcMain.handle('updates:install', () => {
    ...
    const installerPath = updateState.downloadedPath;
