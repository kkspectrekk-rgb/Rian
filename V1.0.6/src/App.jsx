...
function SettingsView({ hasApiKey, onSaved, notify, onOpenAccount, closeAction, onCloseAction, avatar, onAvatar, userName }) {
...
function SearchView({ hasApiKey, onNeedKey, onSelect, activeRequest, quota, onOpenAccount, savedArtists, savedAlbums, onSaveArtist, onSaveAlbum }) {
  ...
  const entityRows = (items, kind) => <div className="entity-grid">{items.map((item) => {
    ...
    return <article className="entity-card" key={item.id}><span className={`entity-art ${kind}`} style={{ backgroundImage: `url(${item.cover || rainIcon})` }} /><div><strong>{item.name}</strong><small>{kind === 'artist' ? `${item.tracks.length} 首匹配歌曲` : item.artist}</small></div><button type="button" className={`entity-save ${saved ? 'saved' : ''}`} aria-label={saved ? '取消收藏' : '收藏到资料库'} onClick={() => (kind === 'artist' ? onSaveArtist : onSaveAlbum)(item)}><Heart size={17} fill={saved ? 'currentColor' : 'none'} /></button></article>;
  })}</div>;
...
function LibraryView({ onOpenLyrics, onImport, onOpenLikes, current, likedCount, avatar, userName, stats }) {
  ...
  return (
    <section className="library-view content-enter">
      <header className="page-heading browse-heading"><div><p>{systemDayLabel(now)} · {now.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false })}</p><h1>现在就听</h1></div><ProfileMenu avatar={avatar} userName={userName} stats={stats} /></header>
...
function EntityLibraryView({ title, subtitle, items, kind, onToggle }) {
  return <section className="entity-library content-enter"><header className="page-heading"><p>你的资料库</p><h1>{title}</h1><span>{items.length} 个收藏</span></header>{items.length ? <div className="library-entity-grid">{items.map((item) => <article className="library-entity-card" key={item.id}><span className={`library-entity-art ${kind}`} style={{ backgroundImage: `url(${item.cover || rainIcon})` }} /><div><strong>{item.name}</strong><small>{item.artist || `${item.tracks?.length || 0} 首歌曲`}</small></div><button className="entity-save saved" type="button" onClick={() => onToggle(item)} aria-label="取消收藏"><Heart size={17} fill="currentColor" /></button></article>)}</div> : <div className="empty-state"><Album /><strong>还没有收藏{title}</strong><span>{subtitle}</span></div>}</section>;
}

function LocalMusicView({ tracks, onImport, onPlay, activeRequest }) {
  const fileRef = useRef(null);
  return <section className="local-music-view content-enter"><header className="page-heading collection-page-heading"><div><p>这台 Windows 设备</p><h1>本地歌曲</h1><span>{tracks.length} 首歌曲</span></div><button className="primary-button" type="button" onClick={() => fileRef.current?.click()}><Upload size={16} />添加文件</button></header><input ref={fileRef} className="sr-only" type="file" accept="audio/*" multiple onChange={onImport} /><TrackRows tracks={tracks} onPlay={onPlay} activeRequest={activeRequest} emptyTitle="还没有本地歌曲" emptyCopy="点击“添加文件”导入 MP3、FLAC、WAV 等音频。" /></section>;
}
...
function App() {
  ...
  const baseView = view === 'search' ? <SearchView hasApiKey={hasApiKey} onNeedKey={needKey} onSelect={selectSearchResult} activeRequest={activeRequest} quota={quota} onOpenAccount={openAccount} savedArtists={savedArtists} savedAlbums={savedAlbums} onSaveArtist={toggleEntity(setSavedArtists)} onSaveAlbum={toggleEntity(setSavedAlbums)} />
    : view === 'settings' ? <SettingsView hasApiKey={hasApiKey} onSaved={setHasApiKey} notify={notify} onOpenAccount={openAccount} closeAction={closeAction} onCloseAction={changeCloseAction} avatar={avatar} onAvatar={setAvatar} userName={quota.userName} />
    : view === 'liked' ? <LikesView liked={liked} onPlay={playSavedTrack} onRemove={toggleLike} activeRequest={activeRequest} />
    : view === 'albums' ? <EntityLibraryView title="专辑" subtitle="在搜索的专辑分类中点击爱心收藏。" items={savedAlbums} kind="album" onToggle={toggleEntity(setSavedAlbums)} />
    : view === 'artists' ? <EntityLibraryView title="歌手" subtitle="在搜索的歌手分类中点击爱心收藏。" items={savedArtists} kind="artist" onToggle={toggleEntity(setSavedArtists)} />
  ...
  return (
    <div className="app-shell" style={rootStyle}>
      ...
      <main className="main-panel">
        <div className="base-view">{baseView}</div>
      ...
      <audio ref={audioRef} onTimeUpdate={handleAudioTimeUpdate} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onEnded={handleEnded} />
