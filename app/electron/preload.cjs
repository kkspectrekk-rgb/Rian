...
contextBridge.exposeInMainWorld('musicBridge', {
  ...
  getWindowAppearance: () => ipcRenderer.invoke('appearance:get'),
  applyWindowAppearance: (value) => ipcRenderer.invoke('appearance:apply', value),
  ...
  qqPlaylist: (id) => ipcRenderer.invoke('qq:playlist', id),
  getCacheInfo: () => ipcRenderer.invoke('cache:get-info'),
  setCachePath: (nextPath) => ipcRenderer.invoke('cache:set-path', nextPath),
  ...
  preparePlayback: (url) => ipcRenderer.invoke('media:prepare-playback', url),
